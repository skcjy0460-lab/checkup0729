...
st.set_page_config(page_title="검진기관 찾기", page_icon="🔍", layout="wide")
...
if search_clicked or "last_results" in st.session_state:
    ...
    if df.empty:
        ...
        ai_col, dl_col = st.columns([1, 1])
        with ai_col:
            ai_clicked = st.button("🤖 AI 추천 코멘트 생성", use_container_width=True)
        with dl_col:
            excel_bytes = se.to_excel_bytes(df)
            st.download_button(
                "📥 검색결과 엑셀 다운로드",
                data=excel_bytes,
                file_name="검진기관_검색결과.xlsx",
        ...
        if len(df) > 50:
            st.caption(f"상위 50건만 표시됩니다. 전체 {len(df):,}건은 엑셀 다운로드로 확인하세요.")
